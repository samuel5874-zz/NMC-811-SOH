...
Material_inputs_for_one_LIB_cell = generate_samples_from_excel("Material inputs for one LIB cell.xlsx")
EF_of_material_inputs_for_one_LIB_cell = generate_samples_from_excel("EF of material inputs for one LIB cell.xlsx")
EE_of_material_inputs_for_one_LIB_cell = generate_samples_from_excel("EE of material inputs for one LIB cell.xlsx")

# Calculate the GHG emissions of material inputs for one LIB cell
# Step 1: Get list of common variables
common_vars = set(Material_inputs_for_one_LIB_cell.keys()) & set(EF_of_material_inputs_for_one_LIB_cell.keys())

# Step 2: Initialize array to hold total GHG emissions for each simulation
GHG_emissions_of_material_inputs_for_one_LIB_cell = np.zeros(n_simulations)
...
for var in common_vars:
    GHG_emissions_of_material_inputs_for_one_LIB_cell += (
        Material_inputs_for_one_LIB_cell[var] * EF_of_material_inputs_for_one_LIB_cell[var]
    )

# Then, separately add the GHG emissions from NMC811
# (amount of NMC811 used × GHG emissions per gram of NMC811)
GHG_emissions_of_material_inputs_for_one_LIB_cell += (
    Material_inputs_for_one_LIB_cell["NMC811"] * GHG_emissions_of_one_gram_of_NMC811
)


# Calculate the embodied energy of material inputs for one LIB cell
# Step 1: Get list of common variables
common_vars = set(Material_inputs_for_one_LIB_cell.keys()) & set(EE_of_material_inputs_for_one_LIB_cell.keys())

# Step 2: Initialize array to hold total GHG emissions for each simulation
Embodied_energy_of_material_inputs_for_one_LIB_cell = np.zeros(n_simulations)
...
for var in common_vars:
    Embodied_energy_of_material_inputs_for_one_LIB_cell += (
        Material_inputs_for_one_LIB_cell[var] * EE_of_material_inputs_for_one_LIB_cell[var]
    )

# Then, separately add the embodied energy of NMC811
# (amount of NMC811 used × embodied energy per gram of NMC811)
Embodied_energy_of_material_inputs_for_one_LIB_cell += (
    Material_inputs_for_one_LIB_cell["NMC811"] * Embodied_energy_of_one_gram_of_NMC811
)
